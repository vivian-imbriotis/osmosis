...
particle* construct_particle(temperature,is_water){
    ...
    p->is_water = is_water;
    if(p->is_water){
        p->pos.x = rand_double()*WATER_X_MAX+WATER_X_MIN;
        p->pos.y = rand_double()*WATER_Y_MAX+WATER_Y_MIN;
        p->mass=WATER_MASS;
    }else{
        p->pos.x = rand_double()*SOLUTE_X_MAX+SOLUTE_X_MIN;
        p->pos.y = rand_double()*SOLUTE_Y_MAX+SOLUTE_Y_MIN;
        p->mass=SOLUTE_MASS;
    }
    p->pos.y = rand_double();
...
vector get_tangent(particle particle1, particle particle2){
    ...
    result.x = particle2.pos.y - particle1.pos.y;
    return normalized(result);
}
